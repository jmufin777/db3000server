...
module.exports = function() {
  ...
  this.log = async function(req) {
    ...
    var ms=(hrTime[0] * 1000000 + hrTime[1] / 1000)
    var hod = (dd.getHours()+'').padStart(2,'0') 
    var min = (dd.getMinutes()+'').padStart(2,'0') 
    var sec = (dd.getSeconds()+'').padStart(2,'0') 
    msString=(ms+'').substr(7,3)
    //300 841 23
  
    //console.log(ms , '  ',msString,' ', hrTime[0],':', hrTime[1], ': ',(hrTime[1]+'').substr(0,3) );
    //console.log('1'.padStart(2,'0'),' ',  hrTime[0] * 1000000 + hrTime[1] / 1000, ' ',ms, ' ', msString )
    
    //fs.writeFileSync("./log/log.txt", `
    fs.appendFileSync(
      "./log/log0.txt",
      `
        ${hod}:${min}:${sec}:${msString};${req.body.txt1.trim()};${req.body.txt2.trim()};${req.body.txt3.trim()};${req.body.txt4.trim()};`,
      { mode: 0o777 }
    );
    ...
    fs.appendFileSync(
      "./log/log.txt",
      `
        ${dd.getHours()}:${dd.getMinutes()}:${dd.getSeconds()};${req.body.txt1.trim()};${req.body.txt2.trim()};${req.body.txt3.trim()};${req.body.txt4.trim()};`,
      { mode: 0o777 }
    );
  };
  this.logS = async function(txt1 = "", txt2 = "", txt3 = "", txt4 = "") {
    ...
    fs.appendFileSync(
      "./log/logS0.txt",
      `
    ${dd.getHours()}:${dd.getMinutes()}:${dd.getSeconds()};${txt1.trim()};${txt2.trim()};${txt3.trim()};${txt4.trim()};`,
      { mode: 0o777 }
    );
  ...
  this.konverze = async function konverze(soubor, idefix, res) {
    ...
    return new Promise(function(resolve) {
      ...
      exec(prikaz2, (error1, stdout1, stderr1) => {
        ...
        if (error1) {
          row = query(
            `insert into prilohy_prijem (nazev,script_sh,thumb_1,thumb_2,idefix_user) select '${soubor}','${prikaz2}','${errfile}','${errfile}',${idefix}`
          );
        } else {
          row = query(
            `insert into prilohy_prijem (nazev,script_sh,thumb_1,thumb_2,idefix_user) select '${soubor}','${prikaz2}','${outfile300}','${outfile800}',${idefix}`
          );
        }
  ...
  this.recFindByExtOrig = function recFindByExtOrig(
    base,
    ext,
    files,
    result //hleda podle pripony - muze se hodit
  ) {
  ...
  this.recFindByExt2 = function recFindByExt2(
    base,
    ext,
    fileinfo,
    files,
    result //Pouzivam , nemazat
  ) {
    files = files || fs.readdirSync(base);
    result = result || [];
  ...
  this.recFindByExt2Bck = function recFindByExt2Bck(
    base,
    ext,
    files,
    result //Pouzivam , nemazat
  ...
  this.pdfRozbor = function pdfRozbor(txt) {
    ...
    console.log(txt);
    fs.appendFileSync(
      "/home/jarda/db3000/server/qUpdate.sql",
      "\n 1 rozbor\n",
      { mode: 0o777 }
    );
    fs.appendFileSync(
