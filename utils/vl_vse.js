...
module.exports = function() {
  ...
  this.create_tmp_zak = async function create_tmp_zak(
    cTable = "tmp",
    idefix_zak = 0
  ) {
    cRet = "";
  ...
  this.create_tmp_nab = async function create_tmp_nab(
    cTable = "tmp",
    idefix_nab = 0
  ) {
    cRet = "";
  ...
  this.get_zak_last = async function get_zak_last(idefix_user) {
    ...
    return new Promise(resolve => {
      row = query(
        `select idefix_zak,idefix_item,obrazovka from zak_log_open where idefix_user=${idefix_user} order by  cas desc limit 1;`
      );
      if (row.length > 0) {
        cRet = row[0];
        //row[0].a.replace(/\{|\}/g,'')
  ...
  this.vl_list = async function vl_list(idefix_zak = 0) {
    ...
    return new Promise(resolve => {
      row = query(
        `select array(select idefix from (select idefix,datumodeslani,vl_znacka from zak_t_vl_v where idefix_zak = ${idefix_zak} and status = 1 order by datumodeslani ) a )::text a `
      );
      if (row.length > 0) {
  ...
  this.vl_set = async function(
    ...
    /*
    idefix_zak: '13634216',                                                               |
        idefix_item: '14077851',                                                              |
        tmpTable: 'calc_my_9_zak18922142898'
        */
    return new Promise(resolve => {
      var ifx = idefix_item;
      if (idefix_zak > 0) {
  ...
  this.vl_unset = async function(
    ...
    return new Promise(resolve => {
      ...
      console.log(
        "VL UNSET //presun do mezi pameti ",
        idefix_zak,
        idefix_item,
        table,
        user
  ...
  this.vl_droppriloha = async function(table, idefix = 0, nPriloha = 1, res) {
    return new Promise(resolve => {
      ...
      row = query(
        `select idefix,obsah,obsah::text>'' as isobsah from ${table} where idefix=${idefix} `
      );
      if (row.length > 0) {
        row.forEach((el, idx) => {
          if (el.isobsah == true) {
            ...
            query(
              `update ${table} set obsah='${JSON.stringify(
                el.obsah
              )}' where idefix=${el.idefix}`
            );
          }
        });
      }
