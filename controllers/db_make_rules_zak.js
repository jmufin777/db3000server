...
const atables = [
  {
    ...
    struct: `
         ...
         vl_id int
         `,
    index_name: [`idefix  ~~~ (idefix)`, `nazev ~~~ (nazev)`],
...
*/
...

  {
    ...
    struct: `
        ...
        radka int default 0 
        `,
    index_name: [`radka  ~~~ (radka)`, `idefix_vl ~~~ (idefix_vl)`],
    reindex: 1,
    initq: [`select 1 ;`]
  },
  {
...
if (false) {
  atables.forEach(e0 => {
    ...
    var cq =
      "insert into db_system (name, struct, index_name,initq, reindex) values ";
    var cq2 = ` 
    ('${e0.name}','${e0.struct}'
    ,array${idx}
    ,array${inittxt}
...
function Indexes(el) {
  ...
  if (el.reindex == 1) {
    try {
      pool.query(
        `select * from pg_indexes where schemaname='public' and tablename ilike '${el.name}'`,
        (err, res) => {
          if (err) {
                  ...
                  pool.query(
                    ...
                    (err, res) => {
                      if (err) {
                        ...
                        console.log(
                          "OK  smazani ",
                          `drop index ${idb.chemaname}.${idb.indexname}`
                        );
                      }
                    }
                  );
