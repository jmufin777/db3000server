...
//      console.log(login)  
  ...
  async insert (req, res, next ) {
    ...
    // return
    try{
      const {kod, sirka_mm } = req.body.form
      ...
      await req.body.form.data.forEach(element => {
        ...
        if (element[0].id < 0 ){
          dotaz = `insert into  ${tabname} (kod,sirka_mm, user_insert_idefix ) values `;
          dotaz += `( ${element[0].kod},'${element[0].sirka_mm}',  login2idefix('${user}')  )`
        }
        if (element[0].id > 0 ){
          dotaz = `update  ${tabname} set kod =${element[0].kod},sirka_mm='${element[0].sirka_mm}', user_update_idefix = login2idefix('${user}')`;
