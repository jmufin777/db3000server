...
const tabname = 'list2_matsubskup'
module.exports = {
...
//      console.log(login)  
  ...
  async insert (req, res, next ) {
    ...
    // return
    try{
      const {kod,idefix_matskup, nazev } = req.body.form
      ...
      await req.body.form.data.forEach(element => {
        ...
        if (element[0].id < 0 ){
          dotaz = `insert into  ${tabname} (kod,nazev,idefix_matskup, user_insert_idefix ) values `;
          dotaz += `( ${element[0].kod},'${element[0].nazev}', '${element[0].idefix_matskup}',  login2idefix('${user}')  )`
        }
        if (element[0].id > 0 ){
          dotaz = `update  ${tabname} set kod =${element[0].kod},nazev='${element[0].nazev}',idefix_matskup='${element[0].idefix_matskup}',  user_update_idefix = login2idefix('${user}')`;
