  ...
  async insert (req, res, next ) {
    ...
    try{
      ...
      await req.body.form.data.forEach(element => {
        ...
        if (element[0].id < 0 ){
          dotaz = 'insert into  list2_barevnost (kod,nazev, user_insert_idefix ) values ';
          dotaz += `( ${element[0].kod},'${element[0].nazev}',  login2idefix('${user}')  )`
        }
        if (element[0].id > 0 ){
          dotaz = `update  list2_barevnost set kod =${element[0].kod},nazev='${element[0].nazev}', user_update_idefix = login2idefix('${user}')`;
